...
def heston_characteristic_function(u, S0, V0, kappa, theta, xi, rho, r, T, j):
    """
    Calcola la funzione caratteristica di Heston per j=1 o j=2.
    """
    x = np.log(S0)
    a = kappa * theta
    
    # Assegnazione dei parametri b e u_i in base alla probabilità j
    if j == 1:
        b = kappa - rho * xi
        u_i = 1j * u
    else:
    ...
    # Calcolo dei coefficienti complessi intermedi (Albrecher et al. formulation)
    d = np.sqrt((rho * xi * u * 1j - b)**2 - xi**2 * (2 * u_i - u**2))
    g = (b - rho * xi * u * 1j + d) / (b - rho * xi * u * 1j - d)
    
    # Componenti C e D della trasformata
    C = r * u * 1j * T + (a / xi**2) * ((b - rho * xi * u * 1j + d) * T - 2 * np.log((1 - g * np.exp(d * T)) / (1 - g)))
    D = ((b - rho * xi * u * 1j + d) / xi**2) * ((1 - np.exp(d * T)) / (1 - g * np.exp(d * T)))
    
    # Restituisce il valore finale della funzione caratteristica complessa
    return np.exp(C + D * V0 + 1j * u * x)

def heston_probability_integrand(u, S0, V0, kappa, theta, xi, rho, r, T, K, j):
    """
    Rappresenta la funzione interna da integrare numericamente.
    """
    cf = heston_characteristic_function(u, S0, V0, kappa, theta, xi, rho, r, T, j)
    # Applica la formula di inversione e restituisce la sola parte reale
    return np.real(np.exp(-1j * u * np.log(K)) * cf / (1j * u))

def heston_call_price(S0, K, T, r, kappa, theta, xi, rho, V0):
    """
    Risolve l'integrale numerico e restituisce il prezzo teorico di una Call.
    """
    # Se l'opzione è già scaduta, il prezzo è semplicemente il valore intrinseco
    if T <= 0:
        return max(0.0, S0 - K)
        
    # Definiamo le funzioni integrande a singola variabile (u) per SciPy
    ...
    # Eseguiamo l'integrazione numerica definita tra un valore vicino a 0 e 100
    P1 = 0.5 + (1 / np.pi) * integrate.quad(int_1, 1e-5, 100, limit=200)[0]
    P2 = 0.5 + (1 / np.pi) * integrate.quad(int_2, 1e-5, 100, limit=200)[0]
    
    # Formula di pricing di Heston in forma chiusa
